...
if [ -n "${CSV_NAME}" ]; then
    ...
    if CR_NAME=$(oc get openstack -n openstack-operators -o name); then
        ...
        To restore it, use:
        oc patch ${CR_NAME} -n openstack-operators --type=merge -p '{\"spec\": {\"operatorOverrides\": [{\"name\": \"swift\", \"replicas\": 1}]}}'\n"

        oc patch ${CR_NAME} -n openstack-operators --type=merge -p '{"spec": {"operatorOverrides": [{"name": "swift", "replicas": 0}]}}'
    fi
fi
